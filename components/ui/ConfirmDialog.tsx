...
export default function ConfirmDialog({
  ...
  return (
    ...
    >
      <h2 className="font-semibold" style={{ fontSize: "1.154rem" }}>{title}</h2>
      {description && (
        <p className="mt-2" style={{ fontSize: "0.923rem", color: "var(--text-dim)" }}>{description}</p>
      )}
      <div className="mt-5 flex justify-end gap-2">
        ...
        <button
          ...
          style={{ fontSize: "0.962rem", background: "var(--bad)" }}
        >
          {confirmLabel}
        </button>
      </div>
